...
2. Suppose the objective of this plot was to show that the proportion of part-time faculty have gone up over time compared to other instructional staff types. **What changes would you propose making to this plot to tell this story?** Implement the changes you think would be most effective, and include the new plot in your report. Document the changes you made and why you made them in a brief paragraph.
...
3. Can you help them improve it? 
- 3.1. First, brainstorm how you would improve it. 
- 3.2. Then create the improved visualization and document your changes/decisions with bullet points. It's ok if _some_ of your improvements are aspirational, i.e. you don't know how to implement it, but you think it's a good idea. Implement what you can and leave notes identifying the aspirational improvements that could not be made. (You don't need to recreate their plots in order to improve them)
...
```{r lab_charts-code-fisheries, eval=FALSE, include=FALSE}
# Potential code for improving the fisheries plot. This is not meant to be a complete solution, but rather a starting point for you to build on and modify as you see fit.
...
## Stretch Yourself with Smokers in Whickham {.unnumbered}

If you still have some time left (or are a graduate student), move on to the remaining exercises below. These are optional stretch goals. They are designed to challenge you and hone your skills.
...
4. What type of study do you think these data come from: observational or experiment? Why?

5. How many observations are in this dataset? What does each observation represent?

6. How many variables are in this dataset? What type of variable is each?
   Display each variable using an appropriate visualization.

7. What would you expect the relationship between smoking status and health outcome to be?

8. Create a visualization depicting the relationship between smoking status and health outcome. Briefly describe the relationship, and evaluate whether this meets your expectations. Additionally, calculate the relevant conditional probabilities to help your narrative. Here is some code to get you started:
...
9. Create a new variable called `age_cat` using the following scheme:

- `age <= 44 ~ "18-44"`
- `age > 44 & age <= 64 ~ "45-64"`
- `age > 64 ~ "65+"`

10. Re-create the visualization depicting the relationship between smoking status and health outcome, faceted by `age_cat`. What changed? What might explain this change? 
...
>  If you want to learn more about this data set,  you can read the original paper describing the phenomenon: [Appleton et al. 1996](https://www.tandfonline.com/doi/abs/10.1080/00031305.1996.10473563). This paper is published in the American Statistician.
...
<details>
  <summary>Click to for hints about the library</summary>
> Hint: You will need to use the ggpattern package to create the pattern fill. The documentation for the package is available [here](https://coolbutuseless.github.io/ggpattern/).

</details>

```{r lab_charts-code-challenge, echo=FALSE, message=FALSE, warning=FALSE}
...
df <- Whickham %>%
  mutate(age_cat = case_when(
    ...
    age >  64 ~ "65+",
    TRUE ~ NA_character_
  )  %>% factor(levels = c("18-44","45-64","65+"))) %>%
  ...
  ungroup() %>%
  mutate(
    xmin = if_else(smoker == "No", 3, 2),
    xmax = if_else(smoker == "No", 4, 3)
  )


# Make the plot
ggplot(df, aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax,fill = smoker)) +
  geom_rect_pattern(
    aes(pattern_fill = outcome,
        pattern_type = smoker),
 ...
 scale_pattern_fill_manual(values = c( "Alive" = "#E67819", "Dead" = "#201a12"))+
   scale_fill_manual(values = c( "Yes" = "black", "No" = "#dbcbb0"))+
 theme_void()
...
If you want to learn more about this data set, you can check out the following resources: appleton1996ignoring
