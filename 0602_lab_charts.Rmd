...
# Make the plot
ggplot(df, aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax, fill = outcome ,
               pattern = smoker)) +
  geom_rect_pattern(aes( pattern_filename = smoker),
    color = "white", linewidth = 0.3,
    
                    pattern = 'placeholder', # use a placeholder pattern to avoid warnings about missing patterns
                   # pattern_type='keanu',
                  #      pattern_fill = "black",      # color of the texture itself
    pattern_alpha = 0.35,        # transparency of texture
  #  pattern_density = 0.5,       # amount of texture
  #  pattern_spacing = 0.03       # spacing between elements
  ) +
  coord_polar(theta = "y") +
  xlim(1.5, 4) +
  facet_wrap(~ age_cat) +
  scale_pattern_filename_manual(values = c("Alive" = "bear", "Dead" = "keanu")) #+
 theme_void()
