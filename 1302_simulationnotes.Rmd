# RDD: Getting Started with Data Simulations in R {#data_simulations}
...
```{r simulations-include-tweet-bowles, echo=FALSE}
"https://twitter.com/RyanPBowles/status/1493978369087524868" %>%
  try_include_tweet()
```
...
The `r` functions for a chosen distribution all work similarly. Other distributions available from the `stats` package include binomial, F, log-normal, beta, exponential, and gamma distributions. More exotic distributions are available through additional packages.
...
As you've likely noticed, each time we run this code, we've gotten different numbers. To ensure reproducibility, we need to *set the seed* with `set.seed()`. Reproducibility means that when running the same code, you get the same results
...
I've seen other people make changes to the mean and standard deviation to create variables that are within a specific range. For example, if the mean is large and the standard deviation is small in relation to the mean, we can generate strictly positive numbers. (I usually use runif() for this, which we'll see below.)
...
## Further Reading on Monte Carlo Simulation



This module has only scratched the surface of data simulation. If you want to go deeper — especially in the context of Monte Carlo simulation studies common in psychological methods research — the following sources are a great place to start.

@fan2012designing is a wonderful introduction to the topic designed specifically for psychology researchers:
...
```{r simulations-include-tweet-mason, echo=FALSE}
"https://twitter.com/SMasonGarrison/status/1495064593508581378" %>%
  try_include_tweet()
```


For a broader tour of the Monte Carlo simulation literature in psychology and related fields, consider exploring:

- @bandalos2012use — use of Monte Carlo methods in structural equation modeling
- @muthen2002how — how to use Monte Carlo simulation to determine sample size and power
- @schoemann2014using — using simulations for planned missing data designs
- @harwell1992summarizing — best practices for summarizing Monte Carlo results in methodological research
- @gonzalez2017new — simulation approaches for complex factor structures
- @hallquist2018introduction — integrating structural equation models with item response theory
- @foldnes2017simple — generating non-normal data with prespecified properties
- @skrondal2000design — design and analysis of Monte Carlo experiments
- @burton2006design — design of simulation studies in medical statistics (broadly applicable)
- @gagne2009misspecification — effects of model misspecification examined via simulation
- @hunt2015evaluating — evaluating statistical packages using simulation

For a fun application of risk quantification concepts related to simulation thinking, see this article on [micromorts](https://nc233.com/2020/03/micromorts-how-much-risk-of-death-would-you-accept/) — a unit for measuring small probabilities of death that illustrates how simulated or modeled quantities translate into real-world meaning.
