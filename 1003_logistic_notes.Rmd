...
```{r logistic_notes-include-1, echo=FALSE, out.width="100%"}
...
```{r logistic_notes-include-2, echo=FALSE}
email %>%
  group_by(spam) %>%
...
```{r logistic_notes-include-3, echo=FALSE}
email %>%
...
```{r logistic_notes-include-4, echo=FALSE, out.width="70%"}
means <- email %>%
  group_by(spam) %>%
  summarize(mean_num_char = mean(num_char)) %>%
  mutate(group = 1)
