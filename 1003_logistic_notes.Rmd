...
```{r logistic_notes-include-content, echo=FALSE, out.width="100%"}
...
```{r logistic_notes-include-content, echo=FALSE}
email %>%
  group_by(spam) %>%
...
```{r logistic_notes-include-content, echo=FALSE, out.width="70%"}
means <- email %>%
  group_by(spam) %>%
  summarize(mean_num_char = mean(num_char)) %>%
  mutate(group = 1)
