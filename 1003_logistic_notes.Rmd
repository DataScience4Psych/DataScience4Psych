# LEC: Notes on Logistic Regression

```{r logistic_notes-setup, include = FALSE}
source("common.R")
...
```{r logistic_notes-code-packages, }
library(openintro)
data(email)
glimpse(email)
```
...
```{r logistic_notes-plot-density, echo=FALSE, out.width="100%"}
...
```{r logistic_notes-calc-mean-chars, echo=FALSE}
email %>%
  group_by(spam) %>%
...
```{r logistic_notes-plot-re-subject, echo=FALSE}
email %>%
...
```{r logistic_notes-plot-jitter, echo=FALSE, out.width="70%"}
means <- email %>%
  group_by(spam) %>%
  summarize(mean_num_char = mean(num_char)) %>%
  mutate(group = 1)
...
```{r logistic_notes-tidy-spam_fit, }
tidy(spam_fit)
```
