...
```{r lab_laquinta-code-1, echo=TRUE, message=FALSE}
library(tidyverse)
library(dsbox)
```
...
```{r lab_laquinta-setup-2, include=FALSE,eval=TRUE}
data(dennys)
data(laquinta)
...
```{r lab_laquinta-code-1-dup2, echo=TRUE,results='hide'}
dn_ak <- dn %>%
  filter(state == "AK")
nrow(dn_ak)
```
...
```{r lab_laquinta-setup-3, include=FALSE,eval=TRUE}
lq_ak <- lq %>%
  filter(state == "AK")
```
...
```{r lab_laquinta-fig-1, echo=FALSE, out.width="300px", fig.cap="Full join of Denny's and La Quinta locations in AK"}
...
```{r lab_laquinta-code-2, echo=TRUE, message=TRUE, warning=FALSE,results='hide'}
...
```{r lab_laquinta-example-1,eval=FALSE}
...
```{r echo=FALSE, message=FALSE, eval=F, warning=FALSE}
dn_lq_ak_mindist %>%
  ggplot(aes(x = closest)) +
  geom_histogram(binwidth = 10, fill = "lightblue", color = "black") +
  labs(
    title = "Distribution of Minimum Distances between Denny's and La Quinta Locations in AK",
