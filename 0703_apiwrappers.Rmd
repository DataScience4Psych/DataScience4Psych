...
```{r apiwrappers-include-1, echo=FALSE}
...
```{r apiwrappers-code-1, message = FALSE, warning = FALSE,error=TRUE}
library(tidyverse)
library(kableExtra)
library(rebird)
```
...
```{r apiwrappers-example-1, eval = FALSE,error=TRUE,include=TRUE}
ebirdkey <- "SECRET API KEY"
```
```{r apiwrappers-setup-2, message=FALSE, warning=FALSE, include=FALSE, error=TRUE}
...
```{r apiwrappers-code-2, eval = TRUE,error=TRUE, cache=TRUE}
...
```{r apiwrappers-code-3, results = 'asis', eval = TRUE,error=TRUE,cache=TRUE}
...
```{r apiwrappers-code-4, eval = TRUE,error=TRUE,cache=TRUE}
...
```{r apiwrappers-code-5, eval = TRUE,error=TRUE,cache=TRUE}
eagle <- ebirdgeo(
  species = "baleag",
...
```{r apiwrappers-code-6, eval = TRUE,error=TRUE,cache=TRUE}
...
```{r apiwrappers-code-7, message = FALSE, warning = FALSE,error=TRUE}
...
```{r apiwrappers-setup-3, include=FALSE,error=TRUE}
...
```{r apiwrappers-code-8, message = FALSE, warning = FALSE, eval = TRUE,error=TRUE}
...
```{r apiwrappers-code-1-dup2, eval = TRUE,error=TRUE}
countryInfo <- geonames::GNcountryInfo()
```

```{r apiwrappers-code-9, eval = TRUE,error=TRUE,cache=TRUE}
glimpse(countryInfo)
```
...
```{r apiwrappers-code-10, eval = TRUE,cache=TRUE,error=TRUE}
francedata <- countryInfo %>%
  filter(countryName == "France")
```

```{r apiwrappers-code-11, eval = TRUE,error=TRUE,cache=TRUE}
...
```{r apiwrappers-code-12, eval = TRUE, cache=TRUE, error=TRUE}
...
```{r apiwrappers-code-2-dup2, eval = TRUE,error=TRUE}
...
```{r apiwrappers-code-13, eval = TRUE,error=TRUE}
nrow(rio_english)
nrow(rio_portuguese)
```
...
```{r apiwrappers-code-3-dup2,error=TRUE}
...
```{r apiwrappers-code-4-dup2,error=TRUE}
gender("Kelsey")
...
```{r apiwrappers-code-5-dup2,error=TRUE, cache=TRUE,error=TRUE}
gender("Mason")
