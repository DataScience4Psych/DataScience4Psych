...
```{r apiwrappers-demo-ebirdregion, eval = TRUE,error=TRUE, cache=TRUE}
...
```{r apiwrappers-demo-ebirdgeo-van, results = 'asis', eval = TRUE,error=TRUE,cache=TRUE}
...
```{r apiwrappers-demo-ebirdgeo-france, eval = TRUE,error=TRUE,cache=TRUE}
...
```{r apiwrappers-demo-ebirdgeo-eagle, eval = TRUE,error=TRUE,cache=TRUE}
eagle <- ebirdgeo(
  species = "baleag",
...
```{r apiwrappers-demo-ebirdgeo-local, eval = TRUE,error=TRUE,cache=TRUE}
...
```{r apiwrappers-demo-geonames-country, eval = TRUE,error=TRUE}
countryInfo <- geonames::GNcountryInfo()
```

```{r apiwrappers-demo-geonames-glimpse, eval = TRUE,error=TRUE,cache=TRUE}
glimpse(countryInfo)
```
...
```{r apiwrappers-demo-france-filter, eval = TRUE,cache=TRUE,error=TRUE}
francedata <- countryInfo %>%
  filter(countryName == "France")
```

```{r apiwrappers-demo-france-cities, eval = TRUE,error=TRUE,cache=TRUE}
...
```{r apiwrappers-demo-france-glimpse, eval = TRUE, cache=TRUE, error=TRUE}
...
```{r apiwrappers-demo-wiki-rio, eval = TRUE,error=TRUE}
...
```{r apiwrappers-demo-wiki-nrow, eval = TRUE,error=TRUE}
nrow(rio_english)
nrow(rio_portuguese)
```
...
```{r apiwrappers-demo-gender-kelsey,error=TRUE}
gender("Kelsey")
...
```{r apiwrappers-demo-gender-mason,error=TRUE, cache=TRUE,error=TRUE}
gender("Mason")
gender("Mason", years = 1940)
```

```{r plotMason, cache=TRUE,error=TRUE}
mason <- gender("Mason")
...
```{r plotCompare, cache=TRUE,error=TRUE}
df %>%
  filter(year_min == year_max) %>%
  ggplot(aes(year_min, proportion_male)) +
  geom_point() +
