...
```{r results = 'asis', eval = TRUE,error=TRUE}
...
```{r eval = TRUE}
eagle <- ebirdgeo(species = "Haliaeetus leucocephalus", lat = 42, lng = -76, key = ebirdkey)
eagle %>%
  head() %>%
  kable()
```
...
```{r eval = TRUE,error=TRUE}
ebirdgeo(species = "Buteo lagopus", key = ebirdkey)
```
