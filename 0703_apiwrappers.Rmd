```{r apiwrappers-setup, include = FALSE}
library(lubridate)
source("common.R")
```
...
# LEC: Use API-wrapping packages {#api-wrappers}

You can follow along with the slides [here][d16b_apis] if you would like to open them full-screen.
<!--
```{r apiing-include-video-lecture, echo=FALSE,eval = knitr::is_latex_output()}
embed_youtube_alt("iy11CqHnfcM")
```
```{r apiing-include-video-lecture-html, echo=FALSE,eval = knitr::is_html_output()}
...
```{r apiing-include-slides-lecture, echo=FALSE}
slide_url(ds4p_urls, "d16b_apis") %>%
  include_url(height = "400px")
```

## ODD: Demystifying APIs with R Wrappers
...
```{r apiwrappers-include-tweet, echo=FALSE}
...
```{r apiwrappers-load-packages, message = FALSE, warning = FALSE,error=TRUE}
library(tidyverse)
library(kableExtra)
library(rebird)
```
...
```{r apiwrappers-ebirdkey, eval = FALSE,error=TRUE,include=TRUE}
ebirdkey <- "SECRET API KEY"
```

```{r apiwrappers-setup-config, message=FALSE, warning=FALSE, include=FALSE, error=TRUE}
ebirdkey_available <- TRUE

if(exists("secrets/ebird_api_key.txt")){
  ebirdkey <- read.table("secrets/ebird_api_key.txt", quote = "\"", comment.char = "")[1, 1]
} else {
  ebirdkey <- Sys.getenv("EBIRD_API_KEY")
  if (ebirdkey == "") {
    source(".Rprofile")
}
}
if (is.null(ebirdkey) || ebirdkey == "") {
  warning("No eBird API key found. Please set your API key in 'secrets/ebird_api_key.txt' or as an environment variable 'EBIRD_API_KEY'.")
  ebirdkey_available <- FALSE
}
```

```{r apiwrappers-demo-ebirdregion, eval = ebirdkey_available,error=TRUE, cache=TRUE}
...
```{r apiwrappers-demo-ebirdgeo-van, results = 'asis', eval = ebirdkey_available,error=TRUE,cache=TRUE}
...
```{r apiwrappers-demo-ebirdgeo-france, eval = ebirdkey_available,error=TRUE,cache=TRUE}
...
```{r apiwrappers-demo-ebirdgeo-eagle, eval = ebirdkey_available,error=TRUE,cache=TRUE}
eagle <- ebirdgeo(
  species = "baleag",
...
```{r apiwrappers-demo-ebirdgeo-local, eval = ebirdkey_available,error=TRUE,cache=TRUE}
ebirdgeo(species = "rolhaw", key = ebirdkey)
```

#### Case Study: Searching geographic info: geonames {#geonames}
...
```{r apiwrappers-load-geonames, message = FALSE, warning = FALSE,error=TRUE}
...
```{r apiwrappers-setup-geonames, include=FALSE,error=TRUE}
geonamesUsername_available <- TRUE
geonamesUsername <- Sys.getenv("GEONAMES_USERNAME")
if (geonamesUsername == "") {
    source(".Rprofile")
}
options(geonamesUsername=geonamesUsername)
if (is.null(getOption("geonamesUsername")) || getOption("geonamesUsername") == "") {
    warning("No GeoNames username found. Please set your username in your .Rprofile with options(geonamesUsername='your_username').")
  geonamesUsername_available <- FALSE
}
```
...
```{r apiwrappers-load-usethis, message = FALSE, warning = FALSE, eval = FALSE, error=TRUE}
...
```{r apiwrappers-demo-geonames-country, eval = geonamesUsername_available, cache = TRUE, error=TRUE}
countryInfo <- geonames::GNcountryInfo()
```

```{r apiwrappers-demo-geonames-glimpse, eval = geonamesUsername_available,error=TRUE,cache=TRUE}
glimpse(countryInfo)
```
...
```{r apiwrappers-demo-france-filter, eval = geonamesUsername_available,cache=TRUE,error=TRUE}
francedata <- countryInfo %>%
  filter(countryName == "France")
```

```{r apiwrappers-demo-france-cities, eval = geonamesUsername_available,error=TRUE,cache=TRUE}
...
```{r apiwrappers-demo-france-glimpse, eval = geonamesUsername_available, cache=TRUE, error=TRUE}
...
```{r apiwrappers-demo-wiki-rio, eval = geonamesUsername_available, cache = TRUE, error=TRUE}
...
```{r apiwrappers-demo-wiki-nrow, eval = geonamesUsername_available, cache = TRUE, error=TRUE}
nrow(rio_english)
nrow(rio_portuguese)
```


### Case Study: Is it a boy or a girl? Gender-associated names throughout US history
...
```{r apiwrappers-load-gender, cache = FALSE, error=TRUE}
...
```{r apiwrappers-demo-gender-kelsey,error=TRUE, cache=TRUE, eval = genderdata_available}
gender("Kelsey")
...
```{r plot, cache=TRUE, error=TRUE, cache=TRUE, eval = genderdata_available}
df <- gender("Kelsey")
...
```{r apiwrappers-demo-gender-mason,error=TRUE, cache=TRUE, eval = genderdata_available}
gender("Mason")
gender("Mason", years = 1940)
```

```{r plotMason, cache=TRUE,error=TRUE, eval = genderdata_available}
mason <- gender("Mason")
...
```{r plotCompare, cache=TRUE,error=TRUE, eval = genderdata_available}
df %>%
  filter(year_min == year_max) %>%
  ggplot(aes(year_min, proportion_male)) +
  geom_point() +
...
```{r links-external, child="includes/links.md"}
```
