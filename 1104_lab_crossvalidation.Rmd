...
The two predictor blanks are `sex` and `pclass`. The `family` argument specifies the error distribution — use `binomial` for logistic regression (binary outcomes). The `data` is already filled in as `titanic3`.
</details>
...
<details>
  <summary>Click for a hint</summary>
Pass your fitted model (`m_apparent`) as the first argument. Use `type = "response"` to get predicted probabilities on the 0–1 scale — without this, `predict()` returns log-odds by default.
</details>
...
<details>
  <summary>Click for a hint</summary>
The blanks are: (1) `p_apparent` (the predicted probabilities from 1.2), (2) `0.5` (the cutoff threshold), (3) `1` (assigned when the condition is TRUE — predicted survivor), and (4) `0` (assigned when FALSE — predicted non-survivor).
</details>
...
<details>
  <summary>Click for a hint</summary>
Use `newdata = titanic_test` when calling `predict()` so the model makes predictions on the held-out passengers. The rest of the steps — applying the 0.5 cutoff and computing accuracy — follow exactly the same pattern as Exercise 2.2.
</details>
...
<details>
  <summary>Click for a hint</summary>
The blanks all refer to the same column — `cv_results$accuracy`. Pass that vector to `mean()`, `sd()`, `min()`, and `max()`.
</details>
...
<details>
  <summary>Click for a hint</summary>
Map `fold` to `x` (wrap in `factor()` so ggplot treats it as discrete) and `accuracy` to `y`. The red dashed line at `mean(cv_results$accuracy)` gives you a visual reference for the average. No blanks to fill — run the code as-is once `cv_results` exists.
</details>
...
<details>
  <summary>Click for a hint</summary>
Inside the loop, `c0` holds the current cutoff. Use `p_test` (from Exercise 2) as your predicted probabilities and compare `yhat_c` against `titanic_test$survived` to compute accuracy.
</details>
...
<details>
  <summary>Click for a hint</summary>
The loop iterates over a fine grid of cutoffs from 0.1 to 0.9. For each cutoff `c0`, apply `ifelse(p_test > c0, 1, 0)` and then compute accuracy against `titanic_test$survived`. The ggplot maps `cutoff` to `x` and `accuracy` to `y`. No blanks to fill — run as-is once `p_test` is available.
</details>
...
<details>
  <summary>Click for a hint</summary>
`f_base` should be `survived ~ sex + pclass`. For `f_expanded`, add the two predictors you chose in 6.1. Store both as formula objects so you can pass them directly into `glm()` later.
</details>
...
<details>
  <summary>Click for a hint</summary>
Filter to rows with non-missing values for each variable in `f_expanded`: `sex`, `pclass`, and the two predictors you added. Using the same filter for both the baseline and expanded models ensures you compare them on identical passengers.
</details>
...
<details>
  <summary>Click for a hint</summary>
Use `rep(1:10, length.out = n())` to create a balanced fold assignment, then wrap it in `sample()` to randomize which passengers fall in each fold. Set any seed you like for reproducibility.
</details>
...
<details>
  <summary>Click for a hint</summary>
The logic inside the function is identical to the loop in Exercise 3.2, with `j`, `formula`, `df`, and `cutoff` replacing their hard-coded equivalents. Fill the blanks with `j` (for fold filtering), `formula` (for `glm()`), `train_j` (data to fit), `test_j` (newdata), `cutoff` (ifelse threshold), and `test_j$survived` (true labels).
</details>
...
<details>
  <summary>Click for a hint</summary>
Pass `f_base` and `f_expanded` (the formula objects from 6.2) into `cv_glm_accuracy()`. Then, inside `data.frame()`, use `cv_base$accuracy` and `cv_exp$accuracy` as the vectors you pass to `mean()` and `sd()`.
</details>
...
<details>
  <summary>Click for a hint</summary>
No blanks to fill — the code is already complete. `mutate(age_missing = ifelse(is.na(age), ...))` creates a grouping variable, and `group_by() %>% summarize()` computes the statistics separately for passengers with and without a recorded age. Run it and compare the two rows.
</details>
...
<details>
  <summary>Click for a hint</summary>
The blanks in the imputation lines all refer to your numeric predictor (e.g., `age`). Compute `mu <- mean(train_j$age, na.rm = TRUE)`, then replace missing values with `mu` in both `train_j$age` and `test_j$age`. The rest of the loop (fold filtering, `glm()`, `predict()`, accuracy) follows the same pattern as Exercise 3.2.
</details>
...
<details>
  <summary>Click for a hint</summary>
No blanks to fill — the code is already complete. `y_j` holds the true labels. TP counts rows where both `yhat_j` and `y_j` equal 1; FP where `yhat_j == 1` but `y_j == 0`; and so on. Sensitivity is TP / (TP + FN) and specificity is TN / (TN + FP).
</details>
...
<details>
  <summary>Click for a hint</summary>
Fill the two blanks with `f_expanded` (the formula) and `train_j` (the data for `glm()`), and `test_j` (newdata for `predict()`). The TP/FP/TN/FN sums and the accuracy/sensitivity/specificity formulas are already complete — no changes needed there.
</details>
...
<details>
  <summary>Click for a hint</summary>
`pivot_longer()` reshapes `cv_confusion` from wide (one column per metric) to long (one row per metric per fold). Map `fold` to `x` (as a factor), `value` to `y`, and `metric` to `fill`. `geom_col(position = "dodge")` places bars side by side for easy comparison. No blanks to fill — run as-is once `cv_confusion` is built.
</details>
