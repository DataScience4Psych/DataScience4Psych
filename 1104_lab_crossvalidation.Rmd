...
```{r lab_crossvalidation-example-example, eval = FALSE}
library(tidyverse)
library(titanic)
library(readxl)
```
...
```{r lab_crossvalidation-code-data, }
...
```{r lab_crossvalidation-code-demo, }
...
```{r lab_crossvalidation-example-dup2-example, order.exercise-2-solution.1, error=TRUE, eval=FALSE}
m_apparent <- glm(
...
```{r lab_crossvalidation-code-demo, exercise-2-solution.1, error=TRUE, eval=T}
m_apparent <- glm(
  survived ~ sex + pclass,
  data = titanic3,
  family = binomial
...
```{r exercise-one-part-one, error=TRUE, eval=FALSE}
set.seed(___)
...
```{r exercise-three-part-three, error=TRUE}
