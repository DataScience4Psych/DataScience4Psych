...
```{r lab_crossvalidation-setup-2, include=FALSE}
...
```{r lab_crossvalidation-example-1, eval = FALSE}
library(tidyverse)
library(titanic)
library(readxl)
```
...
```{r lab_crossvalidation-example-1-dup2, order.exercise2.1, error=TRUE, eval=FALSE}
m_apparent <- glm(
...
```{r lab_crossvalidation-code-3, exercise2.1, error=TRUE, eval=T}
m_apparent <- glm(
  survived ~ sex + pclass,
  data = titanic3,
  family = binomial
