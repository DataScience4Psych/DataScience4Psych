...
Before modeling, we will do one small cleanup: standardize column names to lowercase for titanic3, and ensure survived is numeric (0/1) where needed.
...
We will use `titanic3` again.

```{r load-data, error=TRUE}
titanic3 <- readxl::read_excel(
  "data/titanic3.xls",
  col_types = c(
...
## Exercise 1: Candidate models {.unnumbered}
...
### 1.1 Choose candidate predictors {.unnumbered}

Pick **two** additional predictors from this list:
...
Write down, in 2 to 3 sentences, why you think each predictor should matter for survival.

### 1.2 Define candidate formulas {.unnumbered}
...
```{r exercise1.2solution, include=FALSE, echo=FALSE, error=TRUE}
...
## Exercise 2: Cross validation comparison (listwise deletion) {.unnumbered}
...
```{r exercise2.1solution, include=FALSE, echo=FALSE, error=TRUE}
cv_df <- titanic3 %>%
  filter(!is.na(survived)) %>%
  filter(!is.na(sex)) %>%
  filter(!is.na(pclass)) %>%
...
<details>
  <summary>Click for a solution example</summary>
```{r ref.label = "exercise2.1solution", echo = TRUE, warning = FALSE, message=FALSE}
```
</details>

### 2.2 Create folds {.unnumbered}
...
## Exercise 3: Missingness is a modeling decision {.unnumbered}
...
### 3.1 Quantify the missingness impact {.unnumbered}
...
## Exercise 4: Simple imputation without leakage {.unnumbered}
...
### 4.1 Implement within-fold mean imputation for one variable {.unnumbered}
...
## Exercise 5: Beyond accuracy (confusion matrix) {.unnumbered}

Accuracy treats all mistakes as equally bad. Lloyd’s rarely agrees.

### 5.1 Compute confusion matrix elements on one fold {.unnumbered}
