...
```{r lab_crossvalidation-exercise-one-hint, ref.label="lab_crossvalidation-exercise-one-solution", echo=FALSE, error=TRUE}
```
</details>
...
This model has access to every passenger and their final outcome. It is not being asked to predict the future. It is being asked to summarize the past. The coefficients tell us how survival odds differ by sex and class, but they don't tell us how well the model would predict survival for new passengers. 
...
<details>
  <summary>Starter code</summary>
```{r lab_crossvalidation-predict-apparent-starter, error=TRUE, eval=FALSE}
p_apparent <- predict(___, type = ___)
```
</details>

<details>
  <summary>Click for a hint</summary>
```{r lab_crossvalidation-predict-apparent-hint, ref.label="lab_crossvalidation-predict-apparent", echo=FALSE, error=TRUE}
```
</details>
...
<details>
  <summary>Starter code</summary>
```{r lab_crossvalidation-classify-apparent-starter, error=TRUE, eval=FALSE}
yhat_apparent <- ifelse(___ > ___, ___, ___)
```
</details>

<details>
  <summary>Click for a hint</summary>
```{r lab_crossvalidation-classify-apparent-hint, ref.label="lab_crossvalidation-classify-apparent", echo=FALSE, error=TRUE}
```
</details>
...
<details>
  <summary>Starter code</summary>
```{r lab_crossvalidation-accuracy-apparent-starter, eval=FALSE, error=TRUE}
acc_apparent <- mean(___ == ___, na.rm = TRUE)
```
</details>

<details>
  <summary>Click for a hint</summary>
```{r lab_crossvalidation-accuracy-apparent, include=TRUE, echo=FALSE, error=TRUE}
...
<details>
  <summary>Click for a solution</summary>
```{r ref.label = "lab_crossvalidation-accuracy-apparent", echo = TRUE, warning = FALSE, message=FALSE}
```
</details>

You should get an accuracy of around `r round(acc_apparent,digits=2)`, which looks pretty good at first glance. But remember, this is the model's performance on data it has already seen. It is not a measure of how well the model would perform on new, unseen passengers.
...
Based on these data we correctly predict `r round(acc_apparent*100,digits=2)`% of the passengers. But that doesn't tell us how well the model would have predicted survival for passengers before the disaster. Cross validation exists because this number tells us very little about how well the model would perform before the disaster. Real predictions are made before the ship hits the iceberg.
...
But inconveniently, the test set doesn't have all the variables we'll need later to evaluate the accuracy of our predictions. To get around this, we'll have to do some data management down the road. For now, let's just focus on the mechanics of fitting a model on the training data and evaluating it on the test data.
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-2-solution.1solution, include=T, echo=FALSE, error=TRUE}
m_split <- glm(
  survived ~ sex + pclass,
  data = titanic_train,
  family = binomial
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-2-solution.2solution, echo=FALSE, error=TRUE}
...
Now the real test. We evaluate this model on passengers it has never seen, using `titanic_test`. Notice the crucial difference: we pass `newdata = titanic_test` to `predict()`. This forces the model to make predictions for passengers that played no role in fitting its coefficients. However, the test data doesn't have the outcome variable `survived`, so we can't compute accuracy directly. To get around this, we need extract the outcome variable from the original `titanic3` dataset, which contains all passengers. About `r round(mean(titanic3$name %in% titanic_test$name)*100,digits=2)`% of the passengers in `titanic3` are in the test set. But be careful about how you merge these datasets. You don't want to accidentally give the model access to the true survival outcomes for the test passengers during training. This is called **information leakage** and it can lead to over-optimistic estimates of test accuracy. To avoid this, we will merge `titanic_test` with `titanic3` *after* fitting the model on the training data.
...
Calculate the predicted probabilities, convert them to class predictions using the same cutoff, and compute the test accuracy.
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-2-solution.3.2-hint, ref.label="exercise-2-solution.3.2solution", echo=FALSE, error=TRUE}
```
</details>

<details>
  <summary>Click for a solution</summary>
```{r exercise-2-solution.3.2solution, echo = TRUE, warning = FALSE, message=FALSE}

titanic_test_survival <- titanic_test %>%
  left_join(titanic3 %>% select(name, survived,ticket), by = c("name","ticket"))

p_test <- predict(m_split,
                  newdata = titanic_test_survival,
                  type = "response")
...
```{r exercise-3-solution.1-hint, ref.label="exercise-3-solution.1solution", echo=FALSE, error=TRUE}
```
</details>

<details>
  <summary>Click for a solution</summary>
```{r exercise-3-solution.1solution, echo = TRUE, warning = FALSE, message=FALSE}
set.seed(11)
...
```{r exercise-3-solution.2-hint, ref.label="exercise-3-solution.2solution", echo=FALSE, error=TRUE}
```
</details>

<details>
  <summary>Click for a solution</summary>
```{r exercise-3-solution.2solution, error=TRUE}
cv_results <- data.frame(fold = sort(unique(titanic_cv$fold)), accuracy = NA_real_)
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-3-solution.3-hint, ref.label="exercise-3-solution.3solution", echo=FALSE, error=TRUE}
```
</details>

<details>
  <summary>Click for a solution</summary>
```{r exercise-3-solution.3solution, error=TRUE}
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-3-solution.4-hint, ref.label="exercise-3-solution.4solution", echo=FALSE, error=TRUE}
```
</details>
...
```{r exercise-3-solution.4solution, echo = TRUE, warning = FALSE, message=FALSE}
ggplot(cv_results, aes(x = factor(fold), y = accuracy)) +
  geom_col(fill = "steelblue") +
  geom_hline(yintercept = mean(cv_results$accuracy), linetype = "dashed", color = "red") +
  labs(
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-4-solution.1-hint, ref.label="exercise-4-solution.1solution", echo=FALSE, error=TRUE}
```
</details>
<details>
  <summary>Click for a solution</summary>
```{r exercise-4-solution.1solution, echo=TRUE, error = TRUE}
...
for (i in seq_along(cutoffs)) {
  ...
  cutoff_results$accuracy[i] <- mean(yhat_c == titanic_test_survival$survived, na.rm = TRUE)
...
for (i in seq_along(________)) {
  c0 <- cutoffs_fine[i]
  yhat_c <- ifelse(p_test > c0, 1, 0)
  cutoff_results_fine$accuracy[i] <- mean(_______ == _________, na.rm = TRUE)
}
```
</details>
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-4-solution.2-hint, ref.label="exercise-4-solution.2solution", echo=FALSE, error=TRUE}
```
</details>
...
```{r exercise-4-solution.2solution, echo=TRUE, error=TRUE}
cutoffs_fine <- seq(0.1, 0.9, by = 0.05)
...
for (i in seq_along(cutoffs_fine)) {
  c0 <- cutoffs_fine[i]
  yhat_c <- ifelse(p_test > c0, 1, 0)
  cutoff_results_fine$accuracy[i] <- mean(yhat_c == titanic_test_survival$survived, na.rm = TRUE)
}
...
<details>
  <summary>Starter code</summary>
```{r exercise-5-solution.1starter, error=TRUE, eval=FALSE}
titanic_cv51 <- titanic3 %>%
  filter(!is.na(___)) %>%
  filter(!is.na(___)) %>%
  filter(!is.na(___)) %>%
  filter(!is.na(___))       
...
set.seed(42)
titanic_cv51 <- titanic_cv51 %>%
  mutate(fold = __________________ )

cv_results51 <- data.frame(fold =  ___________, accuracy =  ___________)
for (j in 1:10) {
  train_j <- titanic_cv51 %>% filter(fold != j)
  test_j  <- titanic_cv51 %>% filter(fold == j)
  m_j     <- glm( ___________, data =  ___________, family =  ___________)
  p_j     <- predict( ___________, newdata =  ___________, type = "response")
  yhat_j  <- ifelse( ___________ > 0.5, 1, 0)
  cv_results51$accuracy[j] <- mean(yhat_j ==  ___________, na.rm = TRUE)
}

c(cv_mean = mean(cv_results51$accuracy), cv_sd = sd(cv_results51$accuracy))
```
</details>
...
```{r exercise-5-solution.1solution, echo=TRUE, error=TRUE}
titanic_cv51 <- titanic3 %>%
...
cv_results51 <- data.frame(fold = 1:10, accuracy = NA_real_)
for (j in 1:10) {
  train_j <- titanic_cv51 %>% filter(fold != j)
  test_j  <- titanic_cv51 %>% filter(fold == j)
  m_j     <- glm(survived ~ sex + pclass + age, data = train_j, family = binomial)
  p_j     <- predict(m_j, newdata = test_j, type = "response")
  yhat_j  <- ifelse(p_j > 0.5, 1, 0)
  cv_results51$accuracy[j] <- mean(yhat_j == test_j$survived, na.rm = TRUE)
}
...
<details>
  <summary>Starter code</summary>
```{r exercise-5-solution.2starter, error=TRUE, eval=FALSE}
f_A <- survived ~ sex + pclass
f_B <- survived ~ sex + pclass + ___ + ___

cv_A <- _________(cv_df, formula = ___)
cv_B <- _________(cv_df, formula = ___)

data.frame(
  model   = c("A: base", "B: expanded"),
  cv_mean = c(mean(cv_A$accuracy), mean(cv_B$accuracy)),
  cv_sd   = c(sd(cv_A$accuracy),   sd(cv_B$accuracy))
)
```
</details>
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-5-solution.2-hint, ref.label="exercise-5-solution.2solution", echo=FALSE, error=TRUE}
```
</details>

<details>
  <summary>Click for a solution example</summary>
```{r exercise-5-solution.2solution,   echo=TRUE, error=TRUE}
f_A <- survived ~ sex + pclass
f_B <- survived ~ sex + pclass + age + fare

cv_A <- cv_glm_accuracy(cv_df, formula = f_A)
cv_B <- cv_glm_accuracy(cv_df, formula = f_B)
...
<details>
  <summary>Click for a sample answer</summary>
Good candidate predictors include:

- **`age`**: Children were prioritized during evacuation ("women and children first"), so younger passengers likely had higher survival rates. Age also correlates with physical ability to reach lifeboats.
- **`fare`**: Fare is a proxy for wealth and cabin location. Higher fares corresponded to upper-deck cabins closer to the lifeboats, giving wealthier passengers a structural advantage in evacuation.
- **`embarked`**: Port of embarkation (Cherbourg, Queenstown, Southampton) correlates with passenger demographics and class composition, which in turn predicts survival.

You are not expected to use the same predictors — any two with a reasonable substantive justification are acceptable.
</details>
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-6-solution.2-hint, ref.label="exercise-6-solution.2solution", echo=FALSE, error=TRUE}
```
</details>
...
<details>
  <summary>Click for a solution example</summary>
```{r exercise-6-solution.2solution, echo = TRUE, warning = FALSE, message=FALSE}
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-7-solution.1-hint, ref.label="exercise-7-solution.1solution", echo=FALSE, error=TRUE}
```
</details>
...
<details>
  <summary>Click for a solution example</summary>
```{r exercise-7-solution.1solution, echo = TRUE, warning = FALSE, message=FALSE}
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-7-solution.2-hint, ref.label="exercise-7-solution.2solution", echo=FALSE, error=TRUE}
```
</details>
...
<details>
  <summary>Click for a solution</summary>
```{r exercise-7-solution.2solution, echo = TRUE, warning = FALSE, message=FALSE}
...
This function takes a data frame `df` with a `fold` variable, a model formula, and an optional cutoff. It loops through each fold, fits the specified logistic regression model on the training set, predicts on the test set, and computes accuracy based on the cutoff. The result is a data frame with fold-wise accuracies that can be summarized or compared across models.
...
<details>
  <summary>Click for a solution</summary>
```{r exercise-7-solution.3solution, echo = TRUE, warning = FALSE, message=FALSE}
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-7-solution.4-hint, ref.label="exercise-7-solution.4solution", echo=FALSE, error=TRUE}
```
</details>
...
<details>
  <summary>Click for a solution</summary>
```{r exercise-7-solution.4solution, echo = TRUE, warning = FALSE, message=FALSE}
...
<details>
  <summary>Starter code</summary>
```{r exercise-eight-part-one-starter, error=TRUE, eval=FALSE}
n_all    <- sum(!is.na(titanic3$___))
n_kept   <- nrow(___)
prop_kept <- ___ / ___

c(n_all = ___, n_kept = ___, prop_kept = ___)
```
</details>
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-8-solution.1-hint, ref.label="exercise-8-solution.1solution", echo=FALSE, error=TRUE}
```
</details>

<details>
  <summary>Click for a solution</summary>
```{r exercise-8-solution.1solution, echo = TRUE, warning = FALSE, message=FALSE}
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-8-solution.2-hint, ref.label="exercise-8-solution.2solution", echo=FALSE, error=TRUE}
```
</details>
...
<details>
  <summary>Click for a solution</summary>
```{r exercise-8-solution.2solution, echo = TRUE, warning = FALSE, message=FALSE}
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-9-solution.1-hint, ref.label="exercise-9-solution.1solution", echo=FALSE, error=TRUE}
```
</details>
...
<details>
  <summary>Click for a solution example</summary>
```{r exercise-9-solution.1solution, echo = TRUE, warning = FALSE, message=FALSE}
...
m_j <- glm(formula = f_expanded, data = __________, family = binomial)
p_j <- predict(m_j, newdata = __________, type = ___________)
yhat_j <- ifelse(p_j > 0.5, 1, 0)
y_j <- ________________
...
sensitivity <- _________ / (TP + FN)
specificity <- _______ / (TN + FP)
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-10-solution.1-hint, ref.label="exercise-10-solution.1solution", echo=FALSE, error=TRUE}
```
</details>
...
<details>
  <summary>Click for a solution</summary>
```{r  exercise-10-solution.1solution, echo = TRUE, warning = FALSE, message=FALSE}
...
for (j in cv_confusion$fold) {
  ...
  cv_confusion$accuracy[cv_confusion$fold == j] <- (TP + TN) / (___________)
  cv_confusion$sensitivity[cv_confusion$fold == j] <- TP / (___________)
  cv_confusion$specificity[cv_confusion$fold == j] <- TN / (___________)
}
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-10-solution.2-hint, ref.label="exercise-10-solution.2solution", echo=FALSE, error=TRUE}
```
</details>
...
<details>
  <summary>Click for a solution</summary>
```{r exercise-10-solution.2solution, echo = TRUE, warning = FALSE, message=FALSE}
...
<details>
  <summary>Click for a hint</summary>
```{r exercise-10-solution.3-hint, ref.label="exercise-10-solution.3solution", echo=FALSE, error=TRUE}
```
</details>

<details>
  <summary>Click for a solution</summary>
```{r exercise-10-solution.3solution, echo = TRUE, warning = FALSE, message=FALSE}
