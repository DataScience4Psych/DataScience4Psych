...
We’ve been hired by Lloyd’s of London to help quantify risk in maritime travel. After the Titanic disaster in 1912 and the [claims that followed]((https://www.lloyds.com/titanic)), Lloyd’s wants models that can predict survival outcomes using passenger information. The catch? Lloyd's needs these predictions *before* the ship leaves port, not after we already know who lived and who died.

In this lab, you will build classification models for Titanic survival and discover why apparent accuracy can be dangerously misleading. You'll then evaluate performance more honestly using data splitting and k-fold cross validation, ultimately building toward models that Lloyd's could actually trust.
...
- Compare multiple candidate models using cross-validated accuracy.
- Handle missing data with listwise deletion and simple imputation.
- Avoid information leakage by imputing *within folds*.
- Evaluate classifiers beyond accuracy using confusion matrices, sensitivity, and specificity.
...
Before modeling, we will do two small cleanups: standardize column names to lowercase and create an ordinal-level variable for passenger class.
...
**1.2.** Now use the model to generate predicted survival probabilities for every passenger.
...
```{r lab_crossvalidation-accuracy-apparent, }
...
At this point, the model looks fairly good. In fact, the number looks reassuring. But it answers the wrong question. The model is being evaluated on passengers it already “knows.” This is like asking, after the Titanic sank, whether you can explain who survived. Of course you can... Based on these data we correctly predict `r round(acc_apparent*100,digits=2)`% of the passengers. 
...
```{r exercise-two-part-two-split-model, error=TRUE}
m_split <- glm(
  survived ~ sex + pclass,
  data = titanic_train,
  family = binomial
...
```{r exercise-three-part-three, error=TRUE, eval=FALSE}
...
```{r exercise-four-part-four, error=TRUE, eval=FALSE}
...
```{r exercise-2-solution.4solution, include=FALSE, echo=FALSE, error=TRUE}
...
<details>
  <summary>Click for a solution</summary>
```{r ref.label = "exercise-2-solution.4solution", echo = TRUE, warning = FALSE, message=FALSE}
```
</details>
...
<
```{r lab_crossvalidation-extract-titles, }
...
```{r ref.label = "exercise-1-solution.2solution", echo = TRUE, warning = FALSE, message=FALSE}
```
</details>
...
```{r exercise-seven-part-three-cv-function, error=TRUE}
...
```{r exercise-seven-part-four-compare, error=TRUE}
...
```{r exercise-nine-part-one-imputation, error=TRUE}
...
```{r exercise-ten-part-one-confusion, error=TRUE}
# Choose a fold
j <- 1
