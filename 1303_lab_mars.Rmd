...
```{r exercise1.4solution, error=TRUE} 
set.seed(123)
...
```{r exercise2.2solution, error=TRUE}
#We'll assume that engineers have the highest problem-solving abilities, followed by scientists and medics. We'll create a variable, `problem_solving`, as a function of role, with some random noise added in. 
# Assign problem-solving scores simply
...
Now this approach works well if you already know what you want your model to look like. But what if you want to simulate multiple variables with specific correlations? That's when the `mvrnorm` function comes in handy from the `MASS` package. This function generates data from a multivariate normal distribution, where we can specify means, standard deviations, and the correlation between variables. This is ideal for simulating more complex interdependencies that might exist among colonists' traits, as well as considering the implications for selecting colonists with specific attributes.
...
- Objective: Dive deeper into simulations by generating more complex relationships between variables.
  ...
  the goal is for students to move from a simple univariate simulation to a more complex multivariate simulation, where they can control the relationships between variables. This will help them understand how to simulate more realistic datasets with interdependent attributes. by simulating multiple variables with specific correlations, students can explore sample size calculations, power analysis, and the impact of different relationships on the data. work. the lab will have them see how different choices in the simulation process can affect the results and the implications for their analysis. including policies for selecting colonists with specific attributes.
-->
...
traits_data <- mvrnorm(n = 100, mu = mean_traits, Sigma = cov_matrix, empirical = TRUE)
...
## Exercise 3: Preparing for the Unexpected {.unnumbered}
...
# Simulate 5 different samples of 20 colonists each from our original pool
...
Now, let's dive into these samples. We want to uncover any patterns or inconsistencies that emerge from repeatedly sampling our colonist pool. Are some combinations of skills and demographics more successful than others? Does the distribution of roles affect the group's overall health or technical capabilities?


# Analyzing each sample by summarizing its main attributes
sample_summaries <- lapply(samples, function(sample) {
  summary_stats <- summarize(sample,
    Average_Age = mean(Age),
    SD_Age = sd(Age),
    Average_Health = mean(Health),
    SD_Health = sd(Health),
    Average_TechnicalSkills = mean(TechnicalSkills),
    SD_TechnicalSkills = sd(TechnicalSkills),
    Average_ProblemSolving = mean(ProblemSolving),
    SD_ProblemSolving = sd(ProblemSolving)
  )
