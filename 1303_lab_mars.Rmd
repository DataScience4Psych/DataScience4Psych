...
df_colonists <- data.frame(id = 1:100,
                                      )
```
...
**1.3.**  Add a uniformally-distributed MARSGAR variable to your colonists.
...
### basic method
Let's use age to simulate technical skills. We'll assume that older colonists have higher technical skills, as they have more experience and knowledge. We'll create a variable, `technical_skills` as a function of age, with some random noise added in.
...
df_colonists$technical_skills <- 1 * df_colonists$age + rnorm(100, mean = 0, sd = 5)
...
Now this approach works well if you already know what you want your model to look like. But what if you want to simulate multiple variables with specific correlations? That's when the `mvrnorm` function comes in handy.

One popular method is to use the `mvrnorm()` function from the `MASS` package. This function generates multivariate normal random numbers, allowing us to specify the mean vector, covariance matrix, and number of observations.
...
ggplot(df_colonists, aes(x = ProblemSolving, y = TechnicalSkills)) +
  ...
  labs(title = "Scatter Plot of Colonist ProblemSolving and TechnicalSkillss", x = "ProblemSolving", y = "TechnicalSkillss") +
  theme_minimal()
