...
```{r lab_mars-include-histogram,echo=FALSE,message=FALSE}
set.seed(100)
...
```{r exercise-1-solution.3_examples, error=TRUE}
set.seed(1)
...
```{r lab_mars-code-demo, ref.label = "reptables", echo = TRUE, warning = FALSE}
```
</details>
...
**1.4.**  Add a uniformly-distributed MARSGAR variable to your colony.

```{r exercise-four-part-four, error=TRUE} 
set.seed(123)
...
```{r exercise-1-solution.4solution, error=TRUE,echo=FALSE} 
set.seed(123)
...
```{r lab_mars-include-scatterplot1,echo=FALSE,message=FALSE}
library(viridis)
# Fit a linear model
...
- using multiple steps with `rnorm`, which is a less computationally efficient, but more accessible approach and 
- using `mvrnorm`, which generates data from a multivariate distribution.
...
```{r exercise-two-part-one, error=TRUE}
...
```{r lab_mars-include-scatterplot2,echo=FALSE,message=FALSE, warning=FALSE}
# Fit a linear model
...
<details>
  ...
  If you're stuck, you can check the source code for the solution by examining the following chunk in this corresponding Rmd file exercise-2-solution.2solution.
</details>

<details>
  <summary>Click this to see a solution</summary>
```{r ref.label = "exercise-2-solution.2solution", echo = TRUE, warning = FALSE}
```
</details>

```{r lab_mars-include-density-plot, exercise-2-solution.2plot, error=TRUE, echo=FALSE,warning=FALSE,message=FALSE}
...
```{r exercise-3-solution.2solution, error=TRUE,echo=FALSE, include=FALSE} 
library(MASS)
...
```{r exercise-3-solution.2plot, error=TRUE,echo=FALSE}
# Fit a linear model
...
```
    ...
    ```{r exercise-3-solution.3solution, error=TRUE,include=FALSE, message=FALSE,echo=FALSE}
...
<details>
  <summary>Click this text for a hint about implementing with a for-loop</summary>
    ...
    After running the simulations, you can use the `group_by` and `summarize` functions to get the mean and standard deviation for extraversion and its correlation with openness. 
<details> 
  <summary>Click this text for for-loop based solution</summary>
```{r ref.label = "exercise-4-solutionsolution", echo = TRUE, warning = FALSE, message=FALSE}
```
</details>
</details>
</details>

    ```{r exercise-4-solutionsolution, error=TRUE,include=FALSE, message=FALSE,echo=FALSE}
library(dplyr)
conflicts_prefer(dplyr::select())
set.seed(124)
...
In space colonization, just like in any complex project management, we must prepare for variability and uncertainty. 


To test the resilience of our simulated Mars colony against the chaotic nature of space travel, we'll generate multiple batches of potential colonists at different sample sizes. By examining these various crews, we can determine our optimal colony size (too few humans and one meteor strike could doom us; too many and our life support systems might collapse under the strain!) and analyze how population size affects the summary statistics of our brave Martian pioneers.
...
```{r ref.label = "exercise-5-solutionsolution", echo = FALSE, warning = FALSE, message=FALSE}
```

However, I encourage you to experiment with different plot types and aesthetics to find the best way to visualize the relationship between sample size and covariance. Consider using something that shows the relationship between changes in sample size and the distribution of covariance values.

<details>
  <summary>Click this text for a hint about implementing with for-loop</summary>
    You can use a for loop to repeat the simulation process multiple times. Remember to append the results of each simulation to a single dataset. 
    After running the simulations, you can use the `group_by` and `summarize` functions to get the mean and standard deviation for extraversion and its correlation with openness.

<details> 
  <summary>Click this text for a for-loop based solution</summary>
```{r ref.label = "exercise-5-solutionsolution", echo = TRUE, warning = FALSE, message=FALSE, eval=FALSE}
```
</details>
</details>
    ```{r exercise-5-solutionsolution, error=TRUE,include=FALSE, message=FALSE,echo=FALSE}
library(dplyr)
set.seed(124)
...
Now, another way to do this is to use the `replicate` function, which is a more concise way to repeat the simulation process multiple times. Here's how you can implement it using `replicate`:


```{r exercise-5-solutionsolution-replicate, error=TRUE,include=FALSE, message=FALSE,echo=FALSE}
library(dplyr)
set.seed(124)
sample_sizes <- seq(30, 300, by = 15) # Varying
repetitions_per_condition <- 20 # Number of repetitions for each sample size
mean_skills <- c(50, 50) # Mean technical skills and problem-solving abilities
cov_skills <- matrix(c(100, 50, 50, 100),  ncol = 2) # Covariance matrix for skills
...
for (size in sample_sizes) {
  covariances <- replicate(repetitions_per_condition, {
    skills_data <- mvrnorm(n = size, mu = mean_skills, Sigma = cov_skills, empirical = FALSE)
    cov(skills_data[, 1], skills_data[, 2])
  })

  # Append results
  simulation_results_cov <- rbind(simulation_results_cov, data.frame(
    SampleSize = size,
    Repetition = 1:repetitions_per_condition,
...
<details> 
  <summary>Click this text for a replicate-based solution</summary>
```{r ref.label = "exercise-5-solutionsolution-replicate", echo = TRUE, warning = FALSE, message=FALSE, eval=FALSE}
```
</details>
</details>


```{r ref.label = "exercise-5-solutionsolution-replicate", echo = FALSE, warning = FALSE, message=FALSE}
```


**5.2.** Plot the distributions of some additional statistics from your new empire of colonies, and include an annotation in the plot to show the population parameters.

Now that you've generated multiple colonies with varying sample sizes, you can plot the distributions of additional statistics such as the mean and standard deviation for extraversion, as well as its correlation with openness. Be sure to include annotations in your plots to indicate the population parameters for these statistics, allowing you to visually assess how closely your simulated colonies align with the expected values.



### Pick your favorite sample of colonists from your empire of colonies and watch them thrive on Mars! 

Now, you can select one of the simulated colonies that you find most interesting and we can simulate how they might fare on Mars based on their personality traits and skills. Try using the `simulatePedigree` function from the BGmisc package to create a family tree for your chosen colony. This will allow you to explore how traits might be inherited across generations and how family dynamics could influence the success of your Martian settlers. You can visualize the family tree using the ggpedigree package, which provides a clear and informative way to display genealogical relationships.

```{r, exercise-5-solution.2plot, error=TRUE, echo=FALSE,warning=FALSE,message=FALSE}

library(BGmisc)
library(ggpedigree)
df_colonists_fam <- simulatePedigree(
  kpc = 3,
  Ngen = 5,
  sexR = 0.5,
  marR = 4/5,
...
df_colonists_slim <- df_colonists[1:length(df_colonists_fam$id), ]
df_colonists_slim$colonist_id <- df_colonists_slim$id
df_colonists_slim$id <- df_colonists_fam$id
df_colonists_fam <- left_join(df_colonists_fam, df_colonists_slim, by = "id") %>%
  rename(Resilience = resilience)

ggpedigree(df_colonists_fam, personID  = "id", focal_fill_column  = "Resilience",
           config = list(
             code_male = "M",
             focal_fill_include = TRUE,
             sex_color_include = FALSE,
             label_column = "colonist_id",
             focal_fill_legend_title = "Resilience"
           )) +
  labs(title = "Family Tree of Simulated Mars Colonists",
       subtitle = "Node color represents resilience") +
  # remove legend for shape
  ...
  guides(shape=FALSE, 
         ...
         group=FALSE,
         linetype=FALSE,
         fill=FALSE) # remove legend for shape and add legend for fill
...
<details>
  <summary>Click this text for a hint about implementing the family tree</summary>
    You can use the `simulatePedigree` function from the BGmisc package to create a family tree for your chosen colony. This will allow you to explore how traits might be inherited across generations and how family dynamics could influence the success of your Martian settlers. You can visualize the family tree using the ggpedigree package, which provides a clear and informative way to display genealogical relationships.
</details>
    

<details>
  <summary>Click this text for a solution about implementing the family tree</summary>
  ```{r, exercise-5-solution.2plot, error=TRUE, echo=FALSE,warning=FALSE,message=FALSE}
  ```
</details>
