...
<details>
  <summary>Click this to see a solution</summary>
    ```{r, exercise2.2solution, error=TRUE, echo=FALSE}
...
```{r, exercise2.2plot, error=TRUE, echo=FALSE}
...
Because this is a tad more complex than the previous simulation, I've created some optional scaffolding by structured hints. 
<details>
...
<details>
  <summary>Click this text for a solution</summary>
    ```{r exercise3.3solution, error=TRUE,include=FALSE}
...
<details>
  <summary>Click this text for forloop based solution</summary>
    ```{r, exercise4solution, error=TRUE,echo=TRUE}
...
summary_stats <- simulated_data %>% group_by(rep) %>% 
  select(-colonist_id, -seed) %>% 
  summarise(across(everything(), list(n = count(), mean = mean, sd = sd))) 
