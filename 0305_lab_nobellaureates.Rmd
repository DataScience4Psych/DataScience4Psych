...
# only run when not via github actions
if (Sys.getenv("GITHUB_ACTIONS") != "true"){
tryCatch(source("https://github.com/DataScience4Psych/lab-03-nobel-laureates/raw/refs/heads/main/data/update_data.R"), error = function(e) message("Could not source update_data.R"), silent = TRUE)
}
```
...
The dataset for this assignment can be found as a csv (comma separated values) file in the `data` folder of your repository. You can read it in using the following. (You can also see how I created this data in the `data/update_data.R` script if you're curious.)
...
As a quick exploratory step, let's create a world map that visualizes the number of Nobel laureates by their birth country. 

```{r lab_nobellaureates-setup, message=FALSE, warning=FALSE, include=FALSE}
library(maps)
library(rworldmap)
library(ggplot2)
# A quick look at the data

country_counts <- nobel %>%
  filter(!is.na(born_country_code)) %>%
  count(born_country_code) %>%
  arrange(desc(n)) 
country_counts$n_log <- log10(country_counts$n + 1)
country_counts_map <- joinCountryData2Map(country_counts, joinCode="ISO2", nameJoinColumn="born_country_code") %>% suppressMessages() 


colourPalette <- RColorBrewer::brewer.pal(7,'RdPu')
```

```{r map-laureates-by-country, message=FALSE, warning=FALSE, echo=FALSE}
 
mapCountryData(country_counts_map, nameColumnToPlot="n_log",
                           addLegend=T,
                           mapTitle="Number of Nobel laureates by birth country",
                           oceanCol='lightblue',
...
This map provides a global overview of the distribution of Nobel laureates based on their birth countries. Countries with higher numbers of laureates are represented with darker shades, while those with fewer or no laureates are shown in lighter shades or white. These data have been log-transformed to ensure better visualization across countries with varying counts of laureates. To gain deeper insights, we will now proceed to analyze the data further through a series of exercises.
...
```{r filter-nobel-living, echo=FALSE}
nobel_living <- nobel %>%
  filter(
    !is.na(country),
    gender != "org",
...
```{r exercise-3-solution, error=TRUE, message=FALSE, warning=FALSE, include=FALSE}
