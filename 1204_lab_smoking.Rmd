...
Let's discuss how this test would work. Our goal is to create an approximate null distribution of sample means under the assumption that the true population mean is 7.43 pounds. To do that, we’ll use bootstrapping and then recenter the resulting distribution at the null hypothesis value.
...
- To create an approximate null distribution, recenter the entire distribution so that its mean equals 7.43. 
- This recentering step keeps the estimated spread of the bootstrap distribution the same, but moves its center to the null value.
- (You can do this by subtracting the bootstrap distribution’s mean from each value, and then adding 7.43 back.)
- Once recentered, compare the observed mean to this distribution: calculate the proportion of simulated means that are at least as extreme as the observed mean (i.e., the p-value).

> 📌 This recentered distribution is an approximate null distribution. It is centered at 7.43, as the null hypothesis specifies, while keeping the spread of the bootstrap distribution the same.

4. Run a hypothesis test using simulation-based inference. Your goal is to create an approximate null distribution of sample means under the assumption that the population mean is 7.43 pounds, and then assess how unusual the observed sample mean is relative to that distribution.
...
4b. Recenter the distribution so that its center aligns with the null hypothesis value of 7.43 pounds.
...
4e. Interpret your results. Based on the p-value and your visualization, what do you conclude about whether the mean birth weight differs from 7.43 pounds? Because 7.43 pounds is the benchmark from the 1995 study, explain how that result informs the question of whether birth weight has changed since 1995.
