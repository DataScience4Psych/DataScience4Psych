...
```{r lab_dennys-example-1,eval=FALSE}
install.packages("devtools")
devtools::install_github("rstudio-education/dsbox")
```

```{r lab_dennys-code-1, eval = TRUE, message = FALSE}
library(tidyverse)
library(dsbox)
```
...
```{r lab_dennys-code-1-dup2, eval=TRUE}
...
```{r lab_dennys-code-2, eval=FALSE}
dn %>%
...
```{r lab_dennys-example-2, eval=FALSE}
lq %>%
