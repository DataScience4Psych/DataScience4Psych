...
```{r secrets_happy_graphics-setup, include = FALSE}
source("common.R")
library(gapminder)
library(tidyverse)
```
...
```{r secrets_happy_graphics-code-gapminder-plot}
library(gapminder)
library(tidyverse)
...
```{r secrets_happy_graphics-code-bad-plot}
ggplot(mapping = aes(
  x = year,
  y = life_exp
)) +
...
```{r secrets_happy_graphics-code-cor-example, error = TRUE}
cor(year, lifeExp, data = gapminder)
```

Sure, you can always just repeat the data frame name like so:

```{r secrets_happy_graphics-code-cor-repeat}
...
```{r secrets_happy_graphics-code-with-function}
with(
  gapminder,
  cor(year, lifeExp)
)
...
```{r secrets_happy_graphics-code-magrittr, message = FALSE, warning = FALSE}
library(magrittr)
gapminder %$%
  cor(year, lifeExp)
```
...
```{r secrets_happy_graphics-code-japan-filter}
japan_dat <- gapminder %>%
...
```{r secrets_happy_graphics-example-japan-recap, eval = FALSE}
...
# ODD: Writing figures to file {#save-figs}
...
```{r secrets_happy_graphics-example-ggsave, eval = FALSE}
...
```{r secrets_happy_graphics-code-scale-setup}
library(ggplot2)
library(gapminder)
...
```{r secrets_happy_graphics-setup-config, eval = FALSE, include = FALSE}
...
```{r secrets_happy_graphics-code-list-files}
...
```{r secrets_happy_graphics-code-list-repeat}
list.files(pattern = "^test-fig*")
```
...
```{r scatterplot-lifeExp-vs-year-hidden, echo = FALSE, fig.show = "hide"}
p <- ggplot(
  gapminder,
  aes(
    x = year,
...
```{r secrets_happy_graphics-example-fig-paths, eval = FALSE}
...
```{r secrets_happy_graphics-code-cleanup}
library(fs)
