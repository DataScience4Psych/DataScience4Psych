...
```{r secrets_happy_graphics-code-1, }
library(gapminder)
library(tidyverse)
...
```{r secrets_happy_graphics-code-2, }
ggplot(mapping = aes(
  x = year,
  y = life_exp
)) +
...
```{r secrets_happy_graphics-code-4, }
with(
  gapminder,
  cor(year, lifeExp)
)
...
```{r secrets_happy_graphics-code-5, message = FALSE, warning = FALSE}
library(magrittr)
gapminder %$%
  cor(year, lifeExp)
```
...
```{r secrets_happy_graphics-code-5-dup2, }
japan_dat <- gapminder %>%
...
```{r secrets_happy_graphics-example-1, eval = FALSE}
...
```{r secrets_happy_graphics-example-2, eval = FALSE}
...
```{r secrets_happy_graphics-code-6, }
library(ggplot2)
library(gapminder)
...
```{r secrets_happy_graphics-setup-2, eval = FALSE, include = FALSE}
...
```{r secrets_happy_graphics-code-7, }
...
```{r secrets_happy_graphics-code-8, }
list.files(pattern = "^test-fig*")
```
...
```{r secrets_happy_graphics-example-3, eval = FALSE}
...
```{r secrets_happy_graphics-code-9, }
library(fs)
