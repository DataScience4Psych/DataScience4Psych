...
```{r secrets_happy_graphics-code-plot, }
library(gapminder)
library(tidyverse)
...
```{r secrets_happy_graphics-code-packages, }
with(
  gapminder,
  cor(year, lifeExp)
)
...
```{r secrets_happy_graphics-code-plot, message = FALSE, warning = FALSE}
library(magrittr)
gapminder %$%
  cor(year, lifeExp)
```
...
```{r secrets_happy_graphics-code-dup2-demo, }
japan_dat <- gapminder %>%
...
```{r secrets_happy_graphics-example-example, eval = FALSE}
...
```{r secrets_happy_graphics-code-data, }
...
```{r secrets_happy_graphics-code-demo, }
list.files(pattern = "^test-fig*")
```
