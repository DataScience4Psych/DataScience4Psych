...
```
```{r transformations-code-packages,message=FALSE}
...
```{r transformations-code-packages, }
library(latticeExtra)
un %>%
...
```{r transformations-include-video, echo=FALSE,eval = knitr::is_latex_output()}
embed_youtube_alt("vGOpEpjz2Ks")
```
```{r transformations-include-video, echo=FALSE,eval = knitr::is_html_output()}
