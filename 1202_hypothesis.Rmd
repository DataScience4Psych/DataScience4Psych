...
```{r hypothesis-code-packages, message=FALSE}
## Packages
library(tidyverse)
library(tidymodels)
```
...
```{r hypothesis-include-content, echo=FALSE}
organ_donor <- tibble(
  outcome = c(
    rep("complication", 3),
...
```{r hypothesis-code-demo, message=FALSE}
organ_donor %>%
  count(outcome)
```
...
```{r hypothesis-include-content, echo=FALSE, message=FALSE, warning=FALSE, out.width="50%"}
# set seed
set.seed(20171011)
...
```{r hypothesis-include-content, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r hypothesis-include-content, echo=FALSE, out.width="50%"}
ggplot(data = null_dist, mapping = aes(x = stat)) +
  geom_histogram(binwidth = 0.01) +
  labs(title = "Null distribution")
```
...
```{r hypothesis-code-demo, cache=TRUE}
null_dist <- organ_donor %>%
  specify(
    response = outcome,
    success = "complication"
...
```{r hypothesis-code-plot, cache=TRUE}
ggplot(data = null_dist, mapping = aes(x = stat)) +
  geom_histogram(binwidth = 0.01) +
...
```{r hypothesis-include-content, echo=FALSE, out.width="80%"}
ggplot(data = null_dist, mapping = aes(x = stat)) +
  geom_histogram(binwidth = 0.05) +
  labs(title = "Null distribution")
```
