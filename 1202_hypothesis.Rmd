# ODD: Notes on Hypothesis Testing

```{r hypothesis-setup, include = FALSE}
source("common.R")
```
...
```{r hypothesis-code-packages-load, message=FALSE}
## Packages
library(tidyverse)
library(tidymodels)
```
...
```{r hypothesis-include-organ-donor-data, echo=FALSE}
organ_donor <- tibble(
  outcome = c(
    rep("complication", 3),
...
```{r hypothesis-code-organ-donor-count, message=FALSE}
organ_donor %>%
  count(outcome)
```
...
```{r hypothesis-include-sim1-plot, echo=FALSE, message=FALSE, warning=FALSE, out.width="50%"}
# set seed
set.seed(20171011)
...
```{r hypothesis-include-sim2-plot, echo=FALSE, message=FALSE, warning=FALSE, out.width="50%"}
...
```{r hypothesis-include-sim3-plot, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r hypothesis-code-null-dist-100, }
...
```{r hypothesis-include-null-dist-view, echo=FALSE}
null_dist %>%
  mutate(
...
```{r hypothesis-include-null-hist, echo=FALSE, out.width="50%"}
ggplot(data = null_dist, mapping = aes(x = stat)) +
  geom_histogram(binwidth = 0.01) +
  labs(title = "Null distribution") 
```
...
```{r hypothesis-code-null-dist-1000, cache=TRUE}
null_dist <- organ_donor %>%
  specify(
    response = outcome,
    success = "complication"
...
```{r hypothesis-code-visualize-pvalue, cache=TRUE}
ggplot(data = null_dist, mapping = aes(x = stat)) +
  geom_histogram(binwidth = 0.01) +
  geom_vline(xintercept = 3 / 62, color = "tomato")
...
```{r hypothesis-code-yawn-proportions, }
yawn %>%
  count(group, result) %>%
  group_by(group) %>%
...
```{r hypothesis-include-yawn-proportions-check, echo=FALSE}
yawn %>%
  count(group, result) %>%
  group_by(group) %>%
  mutate(p_hat = n / sum(n))
...
```{r hypothesis-code-yawn-null-dist, }
null_dist <- yawn %>%
  specify(
    response = result,
    explanatory = group,
...
```{r hypothesis-include-yawn-null-hist, echo=FALSE, out.width="80%"}
ggplot(data = null_dist, mapping = aes(x = stat)) +
  geom_histogram(binwidth = 0.05) +
  labs(title = "Null distribution")
```
