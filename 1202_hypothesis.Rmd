...
```{r hypothesis-code-1, message=FALSE}
## Packages
library(tidyverse)
library(tidymodels)
```
...
```{r hypothesis-include-1, echo=FALSE}
organ_donor <- tibble(
  outcome = c(
    rep("complication", 3),
...
```{r hypothesis-code-2, message=FALSE}
organ_donor %>%
  count(outcome)
```
...
```{r hypothesis-include-2, echo=FALSE, message=FALSE, warning=FALSE, out.width="50%"}
# set seed
set.seed(20171011)
...
```{r hypothesis-include-3, echo=FALSE, message=FALSE, warning=FALSE, out.width="50%"}
...
```{r hypothesis-include-4, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r hypothesis-include-5, echo=FALSE}
null_dist %>%
  mutate(
...
```{r hypothesis-include-6, echo=FALSE, out.width="50%"}
ggplot(data = null_dist, mapping = aes(x = stat)) +
  geom_histogram(binwidth = 0.01) +
  labs(title = "Null distribution")
```
...
```{r hypothesis-code-3, cache=TRUE}
null_dist <- organ_donor %>%
  specify(
    response = outcome,
    success = "complication"
...
```{r hypothesis-code-4, cache=TRUE}
ggplot(data = null_dist, mapping = aes(x = stat)) +
  geom_histogram(binwidth = 0.01) +
...
```{r hypothesis-include-7, echo=FALSE}
yawn %>%
  count(group, result) %>%
  group_by(group) %>%
  mutate(p_hat = n / sum(n))
...
```{r hypothesis-include-8, echo=FALSE, out.width="80%"}
ggplot(data = null_dist, mapping = aes(x = stat)) +
  geom_histogram(binwidth = 0.05) +
  labs(title = "Null distribution")
```
