...
```{r read-course-status, message = FALSE, warning = FALSE, echo = FALSE}

ds4p_status <- read_csv("metadata/ds4p_status.csv") %>% select(Module, Name, Slides, Lab, Videos, Activity = Application, `Lecture Notes`) %>%
  mutate(Status = rowMeans(select(., -Module,-Name), na.rm = TRUE) %>%
    round(digits = 3))
```
...
```{r embed-gromit-video,echo=FALSE}
...
```
<!--
```{r embed-gromit-video-html,echo=FALSE, eval = knitr::is_html_output(),error=FALSE}
# Gromit Placing Rail Tracks for 10 Hours
"https://www.youtube.com/watch?v=fwJHNw9jU_U" %>%
  embed_url() %>%
  use_align("center")
```
```{r embed-gromit-video-latex,echo=FALSE, eval = knitr::is_latex_output()}
# Gromit Placing Rail Tracks for 10 Hours
embed_youtube_alt("fwJHNw9jU_U")
```
-->
...
```{r display-status-table, message = FALSE, warning = FALSE, echo = FALSE}
ds4p_status %>%
  DT::datatable(
    rownames = FALSE,
...
```{r generate-package-bib, include = FALSE}
...
```{r display-sitemap-table, message = FALSE, warning = FALSE, echo = FALSE,error=TRUE}
ds4p_urls %>%
  mutate(
...
```{r display-session-platform, message = FALSE,error=TRUE, warning = FALSE, echo = FALSE}
...
```{r display-session-packages, message = FALSE,error=TRUE, warning = FALSE, echo = FALSE}
session$packages %>%
