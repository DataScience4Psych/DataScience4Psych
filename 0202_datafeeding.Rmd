...
```{r datafeeding-setup, include = FALSE}
source("common.R")
```
...
```{r datafeeding-include-1, echo=FALSE}
...
```{r datafeeding-example-1, eval = FALSE}
install.packages("gapminder")
```

Now load the package:

```{r datafeeding-code-1, }
library(gapminder)
```
...
```{r datafeeding-code-2, message = FALSE, warning = FALSE}
str(gapminder)
```
...
```{r datafeeding-example-2, eval = FALSE}
install.packages("tidyverse")
```

Now load it:

```{r datafeeding-code-2-dup2, }
library(tidyverse)
```
