...
The University of Edinburgh Fine Art Collection and the Edinburgh College of Art Collection _"supports the world-leading research and teaching that happens within the University. Comprised of an astonishing range of objects and ideas spanning two millennia and a multitude of artistic forms, the collection reflects not only the long and rich trajectory of the University, but also major national and international shifts in art history."_^[Source: https://collections.ed.ac.uk/art/about]. 


> **Note:** The joint collection contains `r num_pieces` pieces. You can explore them in the sidebar [here](https://collections.ed.ac.uk/art).
...
Go to the course GitHub organization and locate your lab repo, which should be named something to the effect of `lab-08-uoe-art`. Clone the repository and set up your workspace.
...
<details>
  <summary>If you'd like a hint</summary>
Take a look at the help docs for `str_squish()` (with `?str_squish`) 
</details>

```{r lab_art-code-squish-text, }
page %>%
  html_nodes(".iteminfo") %>%
  html_node("h3 a") %>%
  html_text() %>%
...
```{r lab_art-code-function-example, }
add_two <- function(x) {
  x + 2
}
```
...
```{r lab_art-example-skeleton, eval=FALSE}
...
```{r lab_art-code-function-template, }
...
```{r lab_art-example-test-function, eval=FALSE}
...
```{r lab_art-example-map, eval=FALSE}
...
```{r lab_art-include-screenshot, echo=FALSE}
