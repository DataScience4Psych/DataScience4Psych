...
```{r llmapi-code-response-raw, cache=TRUE}
...
```{r llmapi-code-format-text, cache=TRUE, results='asis'}
cat(ai_response)
```
...
```{r llmapi-code-token-usage, cache=TRUE}
...
```{r llmapi-code-error-handling, echo=TRUE}
...
```{r llmapi-code-packages, cache=TRUE}
library(purrr)
prompts <- c(
  "Define p-value",
...
```{r llmapi-include-responses-table, message = FALSE, warning = FALSE, echo = FALSE, cache=TRUE}
...
```{r llmapi-code-activity-prompt, cache=TRUE}
...
```{r llmapi-code-activity-response, cache=TRUE}
