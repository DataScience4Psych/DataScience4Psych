...
In this lab, you will analyze Nobel laureate data to update and extend some of the visualizations from the article. You'll practice data wrangling and visualization using the `tidyverse` packages while exploring the relationship between immigration and scientific recognition. These data were pulled from the [Nobel Prize API](https://nobelprize.readme.io/docs).
...
Confirm that once you have filtered for these characteristics you are left with a data frame with 253 observations, once again using inline code.
...
> Note: Your bar plot won't exactly match the one from the Buzzfeed article. We're using data pulled directly from the Nobel Prize API, which is more up to date than the data used in the Buzzfeed article. So don't worry if your plot looks a little different!
