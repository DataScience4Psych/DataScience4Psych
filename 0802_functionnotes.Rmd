...
We'll begin by loading the `gapminder` dataset, which contains information about data related to countries' populations, gdp per capita, and life expectancy around the world. This dataset provides a rich source of real-world data for practicing data manipulation and analysis
...
## Example Analysis: Max - min

Consider a numeric vector; we want to compute the difference between its max and min. `lifeExp` or `pop` or `gdpPercap` are great examples of a typical input for a function. You can imagine wanting to get this statistic as one of many potentially summary statistics. You might want to do this by year, country, continent, or combinations thereof.

## Get something that works

First, develop some working code for interactive use, using a representative input. I'll use life expectancy as our example variable.
...
```
Now lets go through some natural solutions to get the difference in the max and min of the `lifeExp` variable.
