...
```{r bias_lab-install-fairness, eval=FALSE,include=FALSE}
# Install the fairness package from GitHub
install.packages("devtools")
devtools::install_github("kozodoi/fairness")
```

```{r bias_lab-code-packages, eval = TRUE, message = FALSE}
library(tidyverse)
library(janitor)
```
...
```{r bias_lab-explore-dimensions, message=FALSE, include=FALSE}
...
```{r bias_lab-count-unique, message=FALSE, include=FALSE}
...
```{r bias_lab-plot-scores, eval=FALSE, include=FALSE}
...
```{r bias_lab-plot-demographics, eval=FALSE, include=FALSE}
...
<details>
  <summary>If you need a hint...</summary>
You may want to use case_when() to create a new variable like: "compas_classification" with values: "TP", "TN", "FP", "FN". It can be based on decile_score thresholds and the recidivism outcome
</details>
...
<details>
  <summary>If you need a hint...</summary>
```{r bias_lab-code-filter-recidivists}
...
11. Create a visualization comparing these metrics between Black and white defendants. What disparities do you observe? Why did you chose this style of plot to make thse comparisons?
...
To check calibration, you can calculate recidivism rates by score for each race. This is an advanced analysis - use what you've learned to approach this.
</details>

### Part 5: Designing fairer algorithms {.unnumbered}

14. If you were tasked with creating a fairer risk assessment algorithm, what *specific* changes would you make to address the disparities you've observed? 
...
21. Now build a more complex model that includes race as a predictor and add those predicted probabilities to our dataset.


```{r bias_lab-code-model-with-race, error=TRUE,echo=FALSE} 
...
```{r ref.label = "bias_lab-code-model-with-race", echo = TRUE, warning = FALSE}
```
</details>
