...
Here is a plot of the residual sum of squares as a function of the number of predictors in the model. As we add more predictors, the residual sum of squares decreases. However, after a certain point, adding more predictors does not substantially improve the model fit. Cross-validation can help us identify the optimal number of predictors to include in the model.


```{r cvnotes-code-demo-plot, echo=FALSE}
df <- data.frame(p_vec, rss)

ggplot(df, aes(y=rss, x = p_vec)) +
  geom_line() +
  geom_point() +
  labs(y = "Residual Sum of Squares", x = "Number of Predictors") +
  theme_minimal()
```
...
```{r cvnotes-code-crossval-function, }
...
```
```{r cvnotes-code-crossval-plot, echo=FALSE}
df_cv <- data.frame(p_vec, cv_errors)
ggplot(df_cv, aes(y=cv_errors, x = p_vec)) +
  geom_line() +
  geom_point() +
  labs(y = "Cross-Validated Prediction Error", x = "Number of Predictors") +
  theme_minimal()
