...
df <- read_csv("data/kansas_grouped_rolling_avg.csv") %>%
  mutate(
    date = as.Date(date) + .5, # add 12 hours to make the date labels more accurate
...
df %>%
  ...
  theme(
    ...
    axis.line = element_line(
      color = col_mix("black", "white", 0.80),
      linewidth = 0.5
    )
  ) +
  annotate("text",
    x = date_lo + .5, y = Inf, label = "Mask",
    color = col_mask, vjust = -0.05, hjust = 2.05, size = 2.5
  ) +
  annotate("text",
    x = date_hi - .5, y = Inf, label = "No Mask",
