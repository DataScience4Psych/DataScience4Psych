...
```{r quickgit-include-1, echo=FALSE}
...
```{r quickgit-child-1, child = './supporting-docs/basicsofgithubandgit.Rmd'}
```
...
```{r quickgit-child-2, child = './supporting-docs/installgit.Rmd'}
```

```{r quickgit-child-3, child = './supporting-docs/installgitclient.Rmd'}
```


## Half the battle {#install-intro}

```{r quickgit-child-4, child = './supporting-docs/halfthebattle.Rmd'}
```


### What is a Git client? Why would you want one?

```{r quickgit-child-5, child = './supporting-docs/whygitclient.Rmd'}
```

```{r quickgit-child-6, child = './supporting-docs/installgitclient.Rmd'}
```
...
```{r quickgit-include-1-dup2, echo=FALSE,eval = knitr::is_latex_output()}
embed_youtube_alt("8Dd7KRpKeaE")
```
```{r quickgit-include-2, echo=FALSE,eval = knitr::is_html_output()}
...
```{r quickgit-include-3, echo=FALSE}
