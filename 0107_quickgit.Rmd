...
```{r quickgit-setup, include=FALSE}
source("common.R")
```

```{r embed-twitter-git-humor, echo=FALSE}
...
```{r include-github-basics, child = './supporting-docs/basicsofgithubandgit.Rmd'}
```
...
```{r include-install-git, child = './supporting-docs/installgit.Rmd'}
```

```{r include-install-git-client, child = './supporting-docs/installgitclient.Rmd'}
```


## Half the battle {#install-intro}

```{r include-half-battle, child = './supporting-docs/halfthebattle.Rmd'}
```


### What is a Git client? Why would you want one?

```{r include-why-git-client, child = './supporting-docs/whygitclient.Rmd'}
```

```{r include-install-git-client-video, child = './supporting-docs/installgitclient.Rmd'}
```
...
```{r git-tutorial-video-latex, echo=FALSE,eval = knitr::is_latex_output()}
embed_youtube_alt("8Dd7KRpKeaE")
```
```{r git-tutorial-video-html, echo=FALSE,eval = knitr::is_html_output()}
...
```{r links-quickgit-main, child="admin/md/courselinks.md"}
```
...
```{r embed-ohmygit-game, echo=FALSE}
...
```{r links-quickgit-footer, child="admin/md/courselinks.md"}
```
