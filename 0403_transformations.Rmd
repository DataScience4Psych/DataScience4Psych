...
Let's visualize an attempt to make our regression behave. Below are the original and log-transformed relationships with ggplot2. You've likely used a log-transformation before, but we'll be exploring larger and more general family of transformations.

```{r,warning=FALSE}
ggplot(un, aes(x = GDPperCapita, y = infantMortality)) +
  geom_point(color = "steelblue") +
  labs(title = "Infant Mortality vs. GDP per Capita",
       x = "GDP per Capita", y = "Infant Mortality") +
  theme_bw()

ggplot(un, aes(x = log(GDPperCapita), y = log(infantMortality))) +
  geom_point(color = "steelblue") +
  labs(title = "Log-Transformed Infant Mortality vs. GDP per Capita",
...
The basic idea stems from the fact that functions of the form $y' = y^p, \quad y > 0$ have a graph that is concave up if $p >1$, and concave down if $0<p<1$ (or $p < 0$).  We can use this information to modify the shape of a distribution or the shape of a relationship between variables by using a power transformation ( $p$) on our data. This family of transformations is called Tukey's Ladder of Powers.
