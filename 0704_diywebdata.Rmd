...
```{r diywebdata-code-packages, message = FALSE, warning = FALSE}
library(tidyverse)
```
...
```{r diywebdata-example-example, message = FALSE, warning = FALSE, eval = FALSE}
...
```{r diywebdata-example-example, eval = FALSE}
...
```{r diywebdata-fig-screenshot, echo = FALSE, fig.cap = "From [xkcd](https://imgs.xkcd.com/comics/tags.png)"}
...
```{r diywebdata-include-screenshot, echo = FALSE}
...
```{r diywebdata-fig-screenshot, echo = FALSE, fig.cap = "From [Scimago Journal & Country Rank](https://www.scimagojr.com)"}
