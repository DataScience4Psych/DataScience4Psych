...
```{r diywebdata-include-1, eval = !can_render, echo = FALSE, comment = NA}
...
```{r diywebdata-code-1, message = FALSE, warning = FALSE}
library(tidyverse)
```
...
```{r diywebdata-example-1, message = FALSE, warning = FALSE, eval = FALSE}
...
```{r diywebdata-example-2, eval = FALSE}
...
```{r diywebdata-example-3, eval = FALSE}
## in .Rprofile
...
```{r diywebdata-code-5, message = FALSE, warning = FALSE}
...
```{r diywebdata-code-6, message = FALSE, warning = FALSE}
...
```{r diywebdata-code-7, message = FALSE, warning = FALSE}
...
```{r diywebdata-code-11, message = FALSE, warning = FALSE}
...
```{r diywebdata-fig-1, echo = FALSE, fig.cap = "From [xkcd](https://imgs.xkcd.com/comics/tags.png)"}
...
```{r diywebdata-child-1, child = 'supporting-docs/scraping.Rmd'}
```
...
```{r diywebdata-code-13, message = FALSE, warning = FALSE}
# install.packages("rvest")
library(rvest)
```

```{r diywebdata-example-4, eval = FALSE}
read_html("GapminderHead.html") %>%
  html_table()
```

```{r diywebdata-include-2, echo = FALSE}
...
```{r diywebdata-fig-2, echo = FALSE, fig.cap = "From [Scimago Journal & Country Rank](https://www.scimagojr.com)"}
knitr::include_graphics("img/pubs.png")
```

We can try to get this data directly into R using `read_html()` and `html_table()`:

```{r diywebdata-code-13-dup2, }
...
```{r diywebdata-setup-2, include = FALSE}
