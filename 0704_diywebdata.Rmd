...
```{r diywebdata-setup, include = FALSE}
source("common.R")
...
```{r diywebdata-include-content, eval = !can_render, echo = FALSE, comment = NA}
...
```{r diywebdata-load-tidyverse, message = FALSE, warning = FALSE}
library(tidyverse)
```
...
```{r diywebdata-setup-usethis, message = FALSE, warning = FALSE, eval = FALSE}
...
```{r diywebdata-example-rprofile-option, eval = FALSE}
...
```{r diywebdata-example-custom-key, eval = FALSE}
## in .Rprofile
...
```{r diywebdata-demo-glue-variables, }
...
```{r diywebdata-create-omdb-function, }
...
```{r diywebdata-load-curl, message = FALSE, warning = FALSE}
...
```{r diywebdata-load-jsonlite, message = FALSE, warning = FALSE}
...
```{r diywebdata-load-xml2, message = FALSE, warning = FALSE}
...
```{r diywebdata-demo-xml-contents, }
...
```{r diywebdata-load-httr, message = FALSE, warning = FALSE}
...
```{r diywebdata-fig-xkcd-tags, echo = FALSE, fig.cap = "From [xkcd](https://imgs.xkcd.com/comics/tags.png)"}
...
````markdown
`r xfun::file_string('includes/scraping.Rmd')`
````

Knit the document and click "View in Browser". It should look like this:

```{r diywebdata-child, child = 'includes/scraping.Rmd'}
```
...
```{r diywebdata-load-rvest, message = FALSE, warning = FALSE}
# install.packages("rvest")
library(rvest)
```

```{r diywebdata-demo-read-table, eval = FALSE}
read_html("GapminderHead.html") %>%
  html_table()
```

```{r diywebdata-demo-read-table-real, echo = FALSE}
read_html("includes/GapminderHead.html") %>%
  html_table()
```
...
```{r diywebdata-fig-scimago, echo = FALSE, fig.cap = "From [Scimago Journal & Country Rank](https://www.scimagojr.com)"}
knitr::include_graphics("img/pubs.png")
```

We can try to get this data directly into R using `read_html()` and `html_table()`:

```{r diywebdata-demo-scrape-research, }
...
```{r diywebdata-setup-config, include = FALSE}
