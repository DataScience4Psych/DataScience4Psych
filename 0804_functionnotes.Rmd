...
```
```{r functionnotes-include-1,echo=FALSE}
...
```{r functionnotes-code-13, error = TRUE}
...
```{r functionnotes-code-13-dup2,error=TRUE}
average_delay_by_airline <- function(data = flights, airline_code, delay_type = "dep_delay") {
...
```{r functionnotes-code-14, error = TRUE}
...
```{r functionnotes-code-15, error = TRUE}
mmm2 <- function(x) {
  if (!is.numeric(x)) {
    stop(
...
```{r functionnotes-example-1, eval = FALSE}
...
```{r functionnotes-code-20, error = TRUE}
...
```{r functionnotes-code-24, error = TRUE}
z <- gapminder$lifeExp
z[3] <- NA
quantile(gapminder$lifeExp)
quantile(z)
...
```{r functionnotes-code-25, error = TRUE}
qdiff5 <- function(x,
                   probs = c(0, 1),
                   na.rm = TRUE) {
  stopifnot(is.numeric(x))
...
```{r functionnotes-example-2, eval = FALSE}
library(testthat)
...
```{r functionnotes-setup-2, include = FALSE}
source("common.R")
```
...
```{r functionnotes-code-28, message = FALSE}
library(gapminder)
library(ggplot2)
library(dplyr)
```
...
```{r links-dup2, child="admin/md/courselinks.md"}
```
