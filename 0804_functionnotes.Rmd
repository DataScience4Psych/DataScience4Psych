# RDD: Write your own R functions{#functions-part1}
...
```{r functionnotes-setup, include = FALSE}
source("common.R")
...
```
```{r functionnotes-include-content,echo=FALSE}
...
```{r functionnotes-code-aggregate, }
aggregate(dep_delay ~ carrier, 
          data = flights[flights$carrier == "AA", ], 
          FUN = mean, na.rm = TRUE)$dep_delay
```
...
```{r functionnotes-code-function-basic, }
...
```{r functionnotes-code-test-function, }
...
```{r functionnotes-code-function-data-param, }
...
```{r functionnotes-code-test-flights2, }
flights2 <- flights
...
```{r functionnotes-code-function-enhanced, }
...
```{r functionnotes-code-test-errors, error = TRUE}
...
```{r functionnotes-code-error-handling,error=TRUE}
average_delay_by_airline <- function(data = flights, airline_code, 
                                     delay_type = "dep_delay") {
...
```{r functionnotes-code-stopifnot, error = TRUE}
...
```{r functionnotes-code-if-stop, error = TRUE}
mmm2 <- function(x) {
  if (!is.numeric(x)) {
    stop(
...
# RDD: Where were we? Where are we going?
...
```{r start-function-gapminder-example}
library(gapminder)
```
...
```{r functionnotes-code-restore-mmm, }
mmm <- function(x) {
...
```{r functionnotes-code-quantile-explore, }
...
```{r functionnotes-code-quantile-diff, }
...
```{r functionnotes-code-qdiff2-argnames, }
...
```{r functionnotes-code-show-qdiff1, }
qdiff1
```
...
```{r functionnotes-example-return, eval = FALSE}
...
```{r functionnotes-code-test-missing-probs, error = TRUE}
...
```{r functionnotes-code-qdiff3-defaults, }
...
```{r functionnotes-code-test-qdiff3, }
...
```{r functionnotes-code-validation-exercise, }
...
```{r end-function-gapminder-example}
qdiff3
```
...
```{r start-function-anothergapminder-example}
library(gapminder)
```
...
```{r functionnotes-code-restore-qdiff3, }
qdiff3 <- function(x, probs = c(0, 1)) {
  stopifnot(is.numeric(x))
  the_quantiles <- quantile(x, probs)
  max(the_quantiles) - min(the_quantiles)
...
```{r functionnotes-code-test-na, error = TRUE}
z <- gapminder$lifeExp
z[3] <- NA
quantile(gapminder$lifeExp)
quantile(z)
...
```{r functionnotes-code-qdiff4-hardwire, }
...
```{r functionnotes-code-qdiff5, error = TRUE}
qdiff5 <- function(x,
                   probs = c(0, 1),
                   na.rm = TRUE) {
  stopifnot(is.numeric(x))
...
```{r functionnotes-code-qdiff6-ellipsis, }
...
```{r functionnotes-code-quantile-type, }
set.seed(1234)
z <- rnorm(10)
quantile(z, type = 1)
quantile(z, type = 4)
...
```{r functionnotes-code-test-qdiff6, }
...
```{r functionnotes-example-testthat, eval = FALSE}
library(testthat)
...
```{r end-function-anothergapminder-example, eval = FALSE}
...
# ODD: Function-writing practicum {#functions-practicum}

```{r functionnotes-setup-config, include = FALSE}
source("common.R")
```
...
```{r functionnotes-code-plot, message = FALSE}
library(gapminder)
library(ggplot2)
library(dplyr)
```
...
```{r functionnotes-code-lm-france, }
...
```{r functionnotes-code-le-lin-fit-basic, }
...
```{r functionnotes-code-le-lin-fit-names, }
le_lin_fit <- function(dat, offset = 1952) {
  the_fit <- lm(lifeExp ~ I(year - offset), dat)
...
```{r functionnotes-code-clean-workspace, }
rm(list = ls())
le_lin_fit <- function(dat, offset = 1952) {
  the_fit <- lm(lifeExp ~ I(year - offset), dat)
  setNames(coef(the_fit), c("intercept", "slope"))
