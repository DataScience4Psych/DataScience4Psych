...
```{r plots_to_avoid-code-screenshot, fig.align='center', fig.width=4.5}
...
```{r plots_to_avoid-fig-screenshot, echo=FALSE, eval=TRUE,fig.align='center'}
knitr::include_graphics("img/fig2b.png")
```

Note that even worse that piecharts are donut plots.

```{r plots_to_avoid-fig-screenshot, echo=FALSE, eval=TRUE,fig.align='middle',fig.width=3}
...
```{r plots_to_avoid-code-packages, fig.align="center",fig.width=4,fig.height=4}
# library("downloader")
# tmpfile <- tempfile()
...
```{r plots_to_avoid-code-packages, fig.align="center",fig.width=7,fig.height=4}
...
```{r plots_to_avoid-code-data,warning=FALSE}
...
```{r plots_to_avoid-code-plot,warning=FALSE}
...
```{r plots_to_avoid-code-packages,message=FALSE}
...
```{r plots_to_avoid-code-demo, fig.width=7.5 , fig.height=4, fig.align='center'}
data(mas95)
mypar(1, 2)
...
```{r plots_to_avoid-code-plot,fig.align='center',fig.width=5,fig.height=5}
df_mas95 <- df_mas95 %>%
  mutate(
    x = (r + g) / 2,
    y = (r - g)
...
```{r plots_to_avoid-code-demo,fig.align='center',fig.width=7,fig.height=4}
set.seed(12201970)
before <- runif(6, 5, 8)
...
```{r plots_to_avoid-code-data,fig.align='center',fig.width=5}
...
```{r plots_to_avoid-code-demo,fig.align='center',fig.width=5}
plot(
  x,
  y1,
  ylim = c(0, 1),
...
```{r plots_to_avoid-include-video, echo=FALSE,eval = knitr::is_latex_output()}
embed_youtube_alt("E91bGT9BjYk")
```
```{r plots_to_avoid-include-video, echo=FALSE,eval = knitr::is_html_output()}
...
```{r plots_to_avoid-include-tweet, echo=FALSE}
