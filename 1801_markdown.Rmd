...
R Projects and R Markdown offer a powerful duo for streamlining data analysis and reporting in R. The R Project structure streamlines file access through relative paths, ensuring the integrity of file paths even when the project is relocated. It is designed for seamless integration with version control systems like Git and encourages a tidy workspace by storing data, scripts, and outputs within the project directory. This section takes heavy inspiration from [Zachary M. Smith](https://github.com/zsmith27/rmarkdown_crash-course).
...
leaflet(
  ...
  addCircleMarkers(~long, ~lat,
    ...
    stroke = FALSE,
    popup = paste(
      "Sample ID:", thesis.df$unique_id, "<br/>",
