```{r hands-setup, include = FALSE}
source("common.R")
ds4p_urls <- read.csv("./admin/csv/ds4p_urls.csv")
library(tweetrmd) # ... embedding tweets
```

# LEC: Hands on Data Wrangling {#handson}

## Working with a single data frame

```{r wrangling-video-html, echo=FALSE}
# hands on wrangling
...
```{r embed-wrangling-slides, echo=FALSE}
ds4p_urls %>%
...
```{r load-gapminder-hands, message = FALSE, warning = FALSE}
library(gapminder)
```
...
```{r demonstrate-filter-without-assignment, eval = FALSE}
...
```{r demonstrate-filter-with-assignment, eval = FALSE}
...
```{r count-countries-per-continent, }
my_gap %>%
  group_by(continent) %>%
  summarize(
    n = n(),
...
```{r demonstrate-minmax-rank-filtering, eval = FALSE}
filter(min_rank(desc(lifeExp)) < 2 | min_rank(lifeExp) < 2)
```
