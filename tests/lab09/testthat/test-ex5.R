...
test_that("Ex 5: both recidivism outcomes are present", {
  ...
  expect_true(0 %in% compas$two_year_recid && 1 %in% compas$two_year_recid,
              label = "two_year_recid should contain both 0 (no) and 1 (yes) values")
})
...
test_that("Ex 6: compas has a classification column (TP, TN, FP, FN)", {
  ...
  expect_true(has_classification,
              label = "compas should have a classification column created using decile_score >= 7 (high risk) and <= 4 (low risk)"
  )
})
...
test_that("Ex 6: classification column contains all four categories", {
  ...
  expect_true("TP" %in% class_vals,
              label = "classification should include TP (True Positive: high risk & recidivated)")
  expect_true("TN" %in% class_vals,
              label = "classification should include TN (True Negative: low risk & did not recidivate)")
  expect_true("FP" %in% class_vals,
              label = "classification should include FP (False Positive: high risk & did not recidivate)")
  expect_true("FN" %in% class_vals,
              label = "classification should include FN (False Negative: low risk & recidivated)")
})
...
test_that("Ex 6: classification is consistent with decile_score and two_year_recid", {
  ...
  if (nrow(tp_rows) > 0) {
    expect_true(all(tp_rows$decile_score >= 7),
                label = "TP cases should have decile_score >= 7")
    expect_true(all(tp_rows$two_year_recid == 1),
                label = "TP cases should have two_year_recid == 1")
  }
  ...
  if (nrow(tn_rows) > 0) {
    expect_true(all(tn_rows$decile_score <= 4),
                label = "TN cases should have decile_score <= 4")
    expect_true(all(tn_rows$two_year_recid == 0),
                label = "TN cases should have two_year_recid == 0")
  }
})
