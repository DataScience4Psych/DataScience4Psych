...
test_that("Rmd contains a minimum number of R code chunks", {
  ...
  chunk_starts <- stringr::str_which(.rmd_content, "^```\\{r")
  expect_true(length(chunk_starts) >= 4,
              info = sprintf("Include at least 4 R code chunks in your Rmd file, found %d", length(chunk_starts)))
})

test_that("R code chunks contain actual code (not all empty)", {
  skip_if(length(.rmd_content) == 0)
  chunk_starts <- stringr::str_which(.rmd_content, "^```\\{r")
  chunk_ends <- stringr::str_which(.rmd_content, "^```$")
