...
test_that("Ex 1: nobel has expected key variables", {
  ...
  expect_true(all(expected_cols %in% names(nobel)),
              info = "Make sure nobel contains key variables like id, firstname, surname, year, category, country, gender, born_country, died_date")
})
...
test_that("Ex 1: nobel has at least 6 prize categories", {
  ...
  expect_true(n_cats >= 6,
              info = "Make sure there are at least 6 prize categories in the data")
})
